...
polynomial::polynomial(const polynomial &other) : pol(other.pol) {
...
std::vector<std::pair<power, coeff>> polynomial::canonical_form() const {
    ...
    auto begin = this->pol.rbegin();
    int i = 0;
    while(begin != this->pol.rend()) {
        vec.push_back(std::pair((*begin).first, (*begin).second));
        begin++;
        i++;
    }
    if(vec.empty()) {
...
// void modulo(const polynomial& dividend, const polynomial& divisor) {

// }

polynomial polynomial::operator%(const polynomial& p) const {
    polynomial remainder = *this;
    auto otherMax = p.pol.rbegin();

    while(!remainder.pol.empty() && ((*remainder.pol.rbegin()).first >= (*otherMax).first) != 0) {
        auto currMax = this->pol.rbegin();
        size_t pow = (*currMax).first - (*otherMax).first;
        int coeff = (*currMax).second / (*otherMax).second;
...
void multHelper(const polynomial& split, const polynomial& other, std::map<power,coeff>& subResults) {
    std::map<power,coeff> result;
    std::map<power, coeff> splitMap = split.getMap();
    std::map<power, coeff> otherMap = other.getMap();
    // if(splitMap.size() <= 2 || otherMap.size() <= 2) {
    //     for(auto currval : split.getMap()) {
    //         for(auto pval : otherMap) {
    ...
    polynomial split0;
    polynomial tempsplit1;
    polynomial other0;
    polynomial tempother1;
    polynomial split1;
    polynomial other1;
    ...
    //use otherDeg because it is larger
    size_t otherDeg = other.get_degree();
    ...
    tempsplit1 = polynomial(iterSplit, splitMap.end());
    split1.strip(tempsplit1, otherDeg/2);

    other0 = polynomial(otherMap.begin(), otherSplit);
    tempother1 = polynomial(otherSplit, otherMap.end());
    other1.strip(tempother1, otherDeg/2);
    // other1
    ...
    // if(other1.getMap()[2] == 5896) {
    //     check = true;
    // }

    // std::map<power, coeff> xMap = multHelper(tempSplit, tempOther, subResults);
    // polynomial x = polynomial(xMap.begin(), xMap.end());
    // std::map<power, coeff> yMap = multHelper(split0, other0, subResults);
    // polynomial y = polynomial(yMap.begin(), yMap.end());
    // std::map<power, coeff> zMap = multHelper(split1, other1, subResults);
    // polynomial z = polynomial(zMap.begin(), zMap.end());
...
polynomial polynomial::operator*(const polynomial& p) const {
    ...
    for(int i = 0; i < threads; i++) {
        ...
        thr[i] = std::thread(multHelper, divPoly[i], other, std::ref(subResults[i].pol));
    }
