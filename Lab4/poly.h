...
public:
    ...
    std::unordered_map<power, coeff> getMap() const {return this->pol;}
    ...
    void updateDegree();
    void setDegree(power val) {this->degree = val;};
    private:
        std::unordered_map<power, coeff> pol;
        size_t degree = 0;
};
void multHelper(const polynomial& split, const polynomial& other, std::unordered_map<power,coeff>& result);
